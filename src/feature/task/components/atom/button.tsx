...
export function SubmitButton({ variant = "ghost", ...props }: ButtonProps) {
  const styles = {
    ghost: "bg-transparent hover:bg-zinc-800",
    primary: "bg-lime-300 text-lime-950 hover:bg-lime-400"
  }
  ...
  return (
    <button
      {...props}
      className={`w-full flex items-center justify-center gap-3 px-4 py-2 rounded-lg font-semibold ${styles[variant]}`}
    />
  )
}
